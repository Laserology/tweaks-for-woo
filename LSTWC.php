...
if ( version_compare( PHP_VERSION, '8.0', '<' ) ) {
	add_action( 'admin_notices', function() {
		printf(
			...
			/* translators: %s: PHP version currently running on the server. */
			esc_html__( 'Tweaks for Woo requires PHP 8.0 or later. Your server is running PHP %s', 'tweaks-for-woo' ),
			PHP_VERSION
		);
	} );
	return;
